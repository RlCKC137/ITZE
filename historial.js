  ...
  try{
    ...
    if(adminDoc.exists){
      ...
      authStatus.innerHTML = `
        <p>Este dispositivo todavía no tiene permisos.</p>
        <p>Copia este código y autorízalo desde Firestore (colección <code>admins</code>):</p>
        <div class="device-code">${user.uid}</div>
        <button id="copyUidBtn" type="button">Copiar código</button>
      `;
    ...
    authStatus.textContent = 'No se pudo verificar el acceso: ' + e.message;
  }
}
initAccess();
...
async function renderProfileEditor(username){
  ...
  function attachEvents(){
    ...
    document.getElementById('h-avatarFile').onchange = async e=>{
      const file = e.target.files[0];
      if(!file) return;
      const dataUrl = await resizeImageFile(file, 500, 0.75);
      style.avatar.src = dataUrl;
      style.avatar.posX = 50; style.avatar.posY = 50; style.avatar.zoom = 100;
    ...
    document.getElementById('h-saveBtn').onclick = async ()=>{
      try{
        ...
        console.error(e);
        alert('No se pudo guardar: ' + e.message + ' (si subiste una foto muy grande, prueba con una URL o una imagen más pequeña).');
      }
    };
